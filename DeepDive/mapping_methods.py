...
def score_func(y_true, y_pred, score_type='pearson_r'):
    """
    Returns the scores of y_true and y_pred using the specified score_type.
    Score types: r2, pearson_r, pearson_r2, explained_variance, predicted_values
    """
...
def kfold_regression(X, y, regression, n_splits, score_type, use_tqdm):
    """
    Performs Kfold regression on features and activations, returns the scores.

    :param X: input X (features)
    :param y: input y (targets)
    :param regression: Specify ridge or pls regression
    :param score_type: Specify the scoring type. r2, pearson_r, pearson_r2, explained_variance, predicted_values
    :param use_tqdm: use tqdm progress bar
...
def gcv_ridge_regression(X,y, score_type, alphas = [1.0], return_best_alpha = False):
    ...
    """
    RidgeCV regression with best alpha
    """
...
def neural_regression(feature_map, neural_response, regression = Ridge(alpha=1.0), cv_splits = 5,
    ...
    """
    Runs the neural regression on the feature_maps and neural_response to evaluate the models feature maps. Can use different regression types, cross validation splits and score_types.

    :param feature_map: feature_map from model
    :param neural_response: neural_response of actual neurons to the imagenet_sample
    :param regression: Which regression to use, specify ridge or pls regression or pass in Ridge() object
    :param cv_splits: How many cross validation splits
    :param score_type: Which scoring system to evaluate the regression. Specify from among r2, pearson_r, pearson_r2, explained_variance, or predicted_values.
    :param use_tqdm: Use tqdm to show progress
...
def rdm_regression(target_rdm, model_rdms, regression_type='linear',
    ...
    :param target_rdm: your brain data RDM (n_samples x n_samples)
    :param model_rdms: your model layer RDMs (n_samples x n_samples x n_layers)
    :param n_splits: how many cross_validated folds
    :param n_repeats: how many times to perform k-fold splits
    :param random_state: used if you want to use a particular set of random splits

    :return r: correlation between predicted and actual RDM
    :return  coefficients: the coefficients across k-fold splits
    :return intercepts: the intercepts across k-fold splits
